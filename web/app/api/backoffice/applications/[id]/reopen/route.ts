...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (app.current_status !== "S5_TECH_VALIDATED") {
    return NextResponse.json({ ok: false, error: "Reabrir só é permitido em S5_TECH_VALIDATED." }, { status: 409 });
  }
  ...
  await supabase.from("application_status_history").insert({
    ...
    comment: comment || "Reaberto para a entidade submeter elementos adicionais.",
  });
