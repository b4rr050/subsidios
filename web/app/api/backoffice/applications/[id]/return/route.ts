...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (app.current_status !== "S2_SUBMITTED" && app.current_status !== "S3_IN_REVIEW") {
    return NextResponse.json({ ok: false, error: "Devolver só é permitido em S2_SUBMITTED ou S3_IN_REVIEW." }, { status: 409 });
  }

  const nextStatus = "S4_RETURNED";
  ...
  await supabase.from("application_status_history").insert({
    ...
    comment: comment || "Devolvido à entidade para correções.",
  });
